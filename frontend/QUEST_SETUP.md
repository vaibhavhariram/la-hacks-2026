...
2. Configure tokens (recommended):
   ...
   cp .env.example .env
   ```
   Then set `VITE_CESIUM_ION_TOKEN` (required for terrain/imagery) and optionally `VITE_MAPBOX_TOKEN` (for the Three.js terrain tiles).
...
Update `BASE_URL` in `src/api/client.js`:
```js
const BASE_URL = 'http://192.168.1.X:8000';  // your machine's LAN IP
```

**Option B (deployed backend):**
Set `BASE_URL` to the public deployment URL.
